...
#define IN_RANGE_EQ(a,b)   (((a>=(b-EPSILON))&&(a<=(b+EPSILON)))?1:0)
...
inline double solve_quadratic(double a, double b, double c) {
	double minus_t, plus_t, t;
	if ((b*b - 4*a*c) >= 0) {
		minus_t = (-1*b - sqrt(b*b - 4*a*c)) / (2*a);
		plus_t = (-1*b + sqrt(b*b - 4*a*c)) / (2*a);

		if (minus_t < plus_t && minus_t > 0) {
			t = minus_t;
		} else if (plus_t < minus_t && plus_t > 0) { 
			t = plus_t;
		}
	} else {
		t = -1.0;
