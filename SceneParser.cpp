...
bool SceneParser :: getLightData(int i, SceneLightData& data) 
{
   ...
   data = *m_lights[i];
	// std::cout << "m_lights[" << i << "]->dir.x = " << m_lights[i]->dir[0] << std::endl;
	// std::cout << "m_lights[" << i << "]->dir.y = " << m_lights[i]->dir[1] << std::endl;
	// std::cout << "m_lights[" << i << "]->dir.z = " << m_lights[i]->dir[2] << std::endl;
   return true;
}
